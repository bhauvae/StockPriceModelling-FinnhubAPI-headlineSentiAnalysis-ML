from FINNHUB_API_INFO import FINNHUB_KEY
from data import START_DATE, END_DATE, db_name, dir_path
from ticker_data import stocks as tickers_list
import requests
...
class Init:
    ...
    def __init__(self):
        ...
        -----------
        `self.start.date` : str
            start date of the training period. Must be within the last year for the free version of finnhub. Format
            must be "YYYY-mm-dd"
        `self.end_date` : str
        ...
        if datetime.strptime(self.start_date, "%Y-%m-%d") <= (
            datetime.now() - relativedelta(years=1)
        ):
            raise Exception(
                "'start_date' is older than 1 year. It doesn't work with the free version of finnhub"
            )
...
class FinnHub:
    ...
    def __init__(
        self, start_date, end_date, start_date_, end_date_, tickers, dir_path, db_name
    ):
        """Class constructor
        ...
        `start_date` : str
            ...
            with the free version of finnhub
        `end_date` : str
    ...
    def clean_table(self, conn_, c):
        ...
        c.execute(
            f" DELETE FROM {self.ticker} WHERE {self.news_header[2]} IS NULL OR "
            f"trim({self.news_header[2]}) = '';"
        )
        # remove NULL value from datetime
        c.execute(
            f" DELETE FROM {self.ticker} WHERE {self.news_header[1]} IS NULL OR "
            f"trim({self.news_header[1]}) = '';"
        )

        # removes duplicate entries (row)
        c.execute(
            f" DELETE FROM {self.ticker} WHERE rowid NOT IN (select MIN(rowid)"
    ...
    def create_table(self, conn_, c):
        ...
        for header_ in range(len(self.news_header) - 1):
            c.execute(
                f"alter table {self.ticker} add column '%s' "
                % self.news_header[header_ + 1]
            )
            conn_.commit()
        ...
        for data_ in self.js_data:
            ...
            try:
                c.execute(
                    f"insert into {self.ticker} values (?,?,?,?,?,?,?,?,?)",
                    [
                        data_[self.news_header[0]],
    ...
    def iterate_day(func):
        """Decorator that makes the API call on finnhub each days between the `self.start_date`
        and `self.end_date`"""
...
finnhub = FinnHub(
    start_date=init_.start_date,
    end_date=init_.end_date,
    start_date_=init_.start_date_,
    end_date_=init_.end_date_,
    tickers=init_.tickers,
    dir_path=init_.dir_path,
    db_name=init_.db_name,
)
